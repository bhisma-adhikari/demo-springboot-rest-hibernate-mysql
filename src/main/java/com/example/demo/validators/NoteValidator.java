...
public class NoteValidator extends BaseValidator {
    ...
    public void validateNoteForPost(NoteDto noteDto) throws AppException {
        ...
        if (userId == null) {
            errors.add(AppError.fromAppErrorCode(AppErrorCode.POST_NOTES__USER_ID_MISSING));
        } else {
            UserEntity userEntity = userService.findUser(userId);
            if (userEntity == null) {
                errors.add(AppError.fromAppErrorCode(AppErrorCode.POST_NOTES__USER_ID_NONEXISTING, userId));
            }
        }
